...
(jFiles <- list.files("data"))
file.rename(from = file.path("data", jFiles),
            to = file.path("data", paste0("2012_",jFiles)))

## manually download the 2011 files
(jFiles <- list.files("data", "^[a-zA-z]"))
file.rename(from = file.path("data", jFiles),
            to = file.path("data", paste0("2011_",jFiles)))

## manually download the 2010 files
(jFiles <- list.files("data", "^[a-zA-z]"))
file.rename(from = file.path("data", jFiles),
            to = file.path("data", paste0("2010_",jFiles)))
...
(jFiles <- list.files("data", full.names = TRUE))
file.rename(from = jFiles, to = gsub("(*.csv).txt", "\\1", jFiles))

list.files("data") # aaaand, of course, the capitalization is not consistent
